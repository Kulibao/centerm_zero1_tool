...
# themselves are intentionally kept.
...
# Remove standalone GPU configuration created by fnos_kernel_fix.sh.
if [[ -f /etc/modules-load.d/zero1-gpu.conf ]]; then
  rm -f /etc/modules-load.d/zero1-gpu.conf
  echo "Removed: /etc/modules-load.d/zero1-gpu.conf"
fi

# Remove repair-script backups and temporary output files. These paths are
# generated by the Zero1tool scripts and are not part of the original system.
rm -f /boot/grub/grub.cfg.bak
rm -f /boot/dtb/rockchip/rk3568-nanopi-r5s.dtb.bak_*
rm -f /boot/initrd.img-*.new /boot/uInitrd-*.new
rm -f /tmp/zero1-depmod.log /tmp/zero1-depmod-kmod.log /tmp/zero1-npu-depmod.log
...
rm -f /etc/zero1-custom-mac.conf
rm -f -- /var/log/fan_control.log /var/log/fan_control.log.*
